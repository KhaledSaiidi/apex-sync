...
- `external-dns` values file: add `gitops/apps/external-dns/values.yaml` and wire it from `gitops/apps/external-dns/kustomization.yaml`; set `provider: aws`, `sources: [service]`, `domainFilters: ["${ARGOCD_ENV_base_domain}"]`, `policy: sync`, `registry: txt`, `txtOwnerId: "${ARGOCD_ENV_external_dns_txt_owner_id}"`, `interval: 1m`, `triggerLoopOnEvent: true`, `logLevel: info`, and expose `AWS_ACCESS_KEY_ID` / `AWS_SECRET_ACCESS_KEY` from `route53-credentials-secret`.
