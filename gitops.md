...
- `gitops/apps/istio/istio-gateways/kustomization.yaml` pulls Gateway API from a remote GitHub release URL at render time. Argo repo-server must reach GitHub on every render, and the manifest is outside this repo’s review and rollback boundary. Fix by vendoring the Gateway API manifest into the repo or referencing a pinned local copy instead of a remote URL.
